	...
	for j := 0; j < field.NumField(); j++ {
		...
		case "DataDir":
			...
			if defaults.CanUpdate(field.Field(j).Interface()) {
				dir := fmt.Sprintf("%s-%s", field.Interface().(InstanceSpec).Role(), getPort(field))
				field.Field(j).Set(reflect.ValueOf(filepath.Join(globalOptions.DataDir, dir)))
			}
		case "DeployDir":
			if field.Field(j).String() != "" {
				continue
			...
			if defaults.CanUpdate(field.Field(j).Interface()) {
				dir := fmt.Sprintf("%s-%s", field.Interface().(InstanceSpec).Role(), getPort(field))
				field.Field(j).Set(reflect.ValueOf(filepath.Join(globalOptions.DeployDir, dir)))
			}
		}
	}
